...
const renderWorld = (state) => {
    ...
    if (!world.player.attributes.location)
        throw new Error('player w/ location.');
    if (world.player.attributes.health === undefined)
        throw new Error('player w/ health.');
    if (world.score.attributes.score === undefined)
        throw new Error("actor type 'score' w/ score.");
    ...
    if (world.score.attributes.score > 2000) {
        ...
        if (Boss) {
            if (!Boss.attributes.health)
                throw new Error("Boss w/ health.");
...
const constrainToGrid = (world, gridWidth, gridHeight) => {
    const constrained = world.actors.map((actor) => {
        if (actor.attributes.type !== At.actorType.SCORE) {
            if (actor.attributes.location === undefined)
                throw new Error("actor, which isn't of type score, w/location");
            if ((actor.attributes.type === At.actorType.PLAYERBULLET && actor.attributes.location.y <= 2) ||
    ...
    if (world.player.attributes.location === undefined)
        throw new Error("player w/ locations.");
...
const generateShooterBullet = (world) => {
    const newWorld = { ...world };
    world.actors.forEach(actor => {
        if (actor.attributes.type === At.actorType.SHOOTER) {
            if (!actor.attributes.location)
                throw new Error("Shooter w/location.");
...
const generatePotion = (world, type, gridWidth) => {
...
const generateBoss = (world, gridWidth) => {
    const X = gridWidth / 2;
    const Y = 2;
...
const generateBossBullet = (world, gridWidth) => {
    const newWorld = { ...world };
    world.actors.forEach(actor => {
...
const moveBoss = (world, gridWidth) => {
...
// Input handling
const handleKeyInput = (name, state) => {
    const { world } = state;
    ...
    if (name === 'SPACE' || name === ' ') {
        const bullet = C.make_bullet({ x: world.player.attributes.location.x, y: world.player.attributes.location.y - 1 }, At.actorType.PLAYERBULLET);
...
// Spawner systems
const setupStoneSpawner = (updateState) => {
    const spawnStones = (state) => {
        setTimeout(() => {
...
const setupShooterSpawner = (updateState) => {
    const spawnShooters = (state) => {
        setTimeout(() => {
            updateState((currentState) => {
                ...
                const newShooterSpawnRate = currentState.world.score.attributes.score > 1000 ? 1000 : currentState.shooterSpawnRate;
                return {
                    ...currentState,
                    world: newWorld,
...
const setupShooterBulletSpawner = (updateState) => {
...
const setupPotionSpawner = (updateState) => {
...
const setupBossMode = (updateState) => {
            ...
            updateState((currentState) => {
                if (currentState.isBossMode) {
                    ...
                    if (!hasBoss) {
                        ...
                        cleanedWorld.actors = cleanedWorld.actors.filter(actor => actor.attributes.type === At.actorType.BOSS ||
...
const setupFreezeSystem = (updateState) => {
    const monitorFreezeEffect = () => {
        setTimeout(() => {
            updateState((currentState) => {
                ...
                const hasFreezeContact = currentState.world.actors.some(actor => actor.attributes.type === At.actorType.FREEZE &&
