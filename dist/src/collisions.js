function detectCollisions(actors) {
    ...
    const generatePairs = (xs) => xs.flatMap((a1, i) => xs.slice(i + 1).map(a2 => [a1, a2]));
    // Filter pairs where actors are at the same position
    const areColliding = ([a1, a2]) => {
        const l1 = a1.attributes.location;
        const l2 = a2.attributes.location;
        if (!l1 || !l2)
            throw new Error("areColliding called on pair w/ location");
        return l1.x === l2.x && l1.y === l2.y;
    };
    const haveCollided = ([a1, a2]) => {
        const l1 = a1.attributes.location;
        const l2 = a2.attributes.location;
        if (!l1 || !l2)
            throw new Error("haveCollided called on pair w/ location");
    ...
    const collision = ([a1, a2]) => haveCollided([a1, a2]) || areColliding([a1, a2]);
    // Return the final collisions
    return generatePairs(actors).filter(collision);
}
function handleCollisions(actors) {
